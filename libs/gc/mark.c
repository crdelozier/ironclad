 ...
 *
 * This code has been modified from its original version for use with Ironclad C++.
 *
 */
...
extern void ironclad_mark_address(void * address);
...
// Used to mark from Ironclad C++ smart pointers
mse * ironclad_mark_stack_top;
mse * ironclad_mark_stack_limit;
                            ...
                            mse *mark_stack_limit)
{
  ...
  ptr_t limit;  /* (Incl) limit of current candidate    */
                                /* range                                */
  word descr;
  ptr_t greatest_ha = (char*)GC_greatest_plausible_heap_addr;
  ptr_t least_ha = (char*)GC_least_plausible_heap_addr;
  DECLARE_HDR_CACHE;
...
# endif
  {
    ...
    //printf("current p: %p, mstop: %p\n",(void*)current_p,(void*)mark_stack_top);
...
#               endif /* ENABLE_TRACE */
		//GC_log_printf("Marking: %p from pointer %p\n",(ptr_t)current,current_p);
...
#         endif /* ENABLE_TRACE */
        ...
        case GC_DS_PER_OBJECT:
          ...
          goto retry;
      case GC_DS_PRECISE:
        // current_p is the base of the allocation
        // These allocations are precisely marked 
        mark_stack_top--;

        if(current_p == (void*)mark_stack_limit){
          continue;
        }

        ironclad_mark_stack_top = mark_stack_top;
        ironclad_mark_stack_limit = mark_stack_limit;

        // Call the virtual mark() method for this allocation
        ironclad_mark_address((void*)current_p);

        mark_stack_top = ironclad_mark_stack_top;
        mark_stack_limit = ironclad_mark_stack_limit;
                                                ...
                                                void **src)
{
    ...
    if (EXPECT(IS_FORWARDING_ADDR_OR_NIL(hhdr), FALSE)) {
      if (GC_all_interior_pointers) {
        hhdr = GC_find_header(((char*)GC_base(obj)));
        if (hhdr == 0) {
    ...
    PUSH_CONTENTS_HDR(((char*)obj), mark_stack_ptr /* modified */, mark_stack_limit,
...
GC_INNER void GC_push_all_eager(ptr_t bottom, ptr_t top)
{
    ...
    register word q;
    register word *lim;
    register ptr_t greatest_ha = (char*)GC_greatest_plausible_heap_addr;
    register ptr_t least_ha = (char*)GC_least_plausible_heap_addr;
...
inline void ironclad_precise_mark(void* current, void** source)
{ 
  ironclad_mark_stack_top = GC_mark_and_push(current,
                                             ironclad_mark_stack_top,
                                             ironclad_mark_stack_limit,
                                             source);
}
